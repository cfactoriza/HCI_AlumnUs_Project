...
const defaultPeerNotes = [
  ...
  [],
  [{ text: "Consider discussing time management improvements.", by: "Mentor" }, { text: "I had a similar experience first year!", by: "Reviewer - Chris" }],
  [],
...
function cloneInitialEvents() {
  return initialEvents.map(ev => ({ ...ev, isGoal: ev.isGoal || false, completed: false }));
}

function createState() {
  return { events: cloneInitialEvents(), peerNotes: JSON.parse(JSON.stringify(defaultPeerNotes)), changeHistory: [] };
}
...
function renderHistory() {
  ...
  if (historyDiv) {
    historyDiv.innerHTML = "<h3>Change History (last 10)</h3><ul>" + state.changeHistory.map(h => `<li>${h}</li>`).join("") + "</ul>";
  }
}
...
function getEventTags(ev, idx, arr) {
  ...
  if (ev.type === "graduation" && idx !== 0) {
    ...
    if (!prevTypes.includes("general") && !prevTypes.includes("job")) tags.push("Check graduation placement");
  }
...
function renderTimeline() {
  ...
  state.events.forEach((ev, idx) => {
    ...
    const tagsSection = eventTags.length ? `<div class="event-tags">${eventTags.map(t => `<span class="tag">${t}</span>`).join(' ')}</div>` : "";
    ...
    const goalSection = isGoal
      ? `<div class="goal-badge">${ev.completed ? "✔ Goal Complete" : "🎯 Milestone/Goal"}</div>
         ${!ev.completed ? `<button onclick="markGoalComplete(${idx})" class="goal-btn action-btn">Mark Complete</button>` : ""}`
      : "";
    const peerNoteBtn = `<button onclick="addPeerNote(${idx})" class="peer-btn action-btn">Leave Reviewer Note</button>`;
    const notesSection = state.peerNotes[idx].length
      ? `<div class="peer-notes">${
          state.peerNotes[idx].map(note =>
            note.by === "Mentor"
      ...
      : "";
    const pinSection = `<button onclick="pinNotePrompt(${idx})" class="pin-btn action-btn">Pin Note</button>`;
    const eventNode = document.createElement('div');
    eventNode.className = `event${highlight ? ` ${highlight}` : ''} ${goalClass}`;
    eventNode.innerHTML = `
      <div class="event-content">
        <img src="${imgUrl}" class="event-image-bg">
        ${goalSection}
        <div class="event-date">${ev.date}</div>
        ${tagsSection}
        <div class="event-des" id="event-des-${idx}">${ev.description}</div>
        <button onclick="editEvent(${idx})" class="edit-btn action-btn">Edit</button>
        <button onclick="deleteEvent(${idx})" class="delete-btn action-btn">Delete</button>
        ${idx > 0 ? `<button onclick="moveEventUp(${idx})" class="move-btn action-btn">Move Up</button>` : ''}
        ${idx < state.events.length - 1 ? `<button onclick="moveEventDown(${idx})" class="move-btn action-btn">Move Down</button>` : ''}
        ${pinSection}
        ${peerNoteBtn}
        ${notesSection}
      </div>
...
window.editEvent = function(idx) {
  ...
  descElem.innerHTML = `<input type="text" id="desc-input-${idx}" value="${state.events[idx].description}" /><button onclick="saveEvent(${idx})" class="save-btn action-btn">Save</button>`;
};
...
window.deleteEvent = function(idx) {
  const eventDesc = state.events[idx]?.description || "";
  const confirmDelete = confirm(`Are you sure you want to delete:\n"${eventDesc}" ?`);
  if (!confirmDelete) return;
  logChange("Deleted event: " + eventDesc);
